...
public class BobberEntity extends ProjectileEntity {
    private static final Logger LOGGER = LogUtils.getLogger();
    private final Random velocityRandom = Random.create();
    private boolean caughtFish;
    private int outOfOpenWaterTicks;
    private static final int field_30665 = 10;
    private static final TrackedData<Integer> HOOK_ENTITY_ID = DataTracker.registerData(Fishcord.fishcordmods.entity.BobberEntity.class, TrackedDataHandlerRegistry.INTEGER);
    private static final TrackedData<Boolean> CAUGHT_FISH = DataTracker.registerData(Fishcord.fishcordmods.entity.BobberEntity.class, TrackedDataHandlerRegistry.BOOLEAN);
    ...
    private Fishcord.fishcordmods.entity.BobberEntity.State state = Fishcord.fishcordmods.entity.BobberEntity.State.FLYING;
    ...
    public BobberEntity(EntityType<? extends ProjectileEntity> type, World world, int luckBonus, int waitTimeReductionTicks) {
        super(type, world);
        this.luckBonus = Math.max(0, luckBonus);
        this.waitTimeReductionTicks = Math.max(0, waitTimeReductionTicks);
    }

    public BobberEntity(EntityType<? extends ProjectileEntity> entityType, World world) {
        this(entityType, world, 0, 0);
    }

    public BobberEntity(PlayerEntity thrower, World world, int luckBonus, int waitTimeReductionTicks) {
        this(ModEntities.BOBBER, world, luckBonus, waitTimeReductionTicks);
    ...
    public void tick() {
        ...
        if (playerEntity == null) {
            ...
            boolean bl = f > 0.0F;
            if (this.state == Fishcord.fishcordmods.entity.BobberEntity.State.FLYING) {
                if (this.hookedEntity != null) {
                    this.setVelocity(Vec3d.ZERO);
                    this.state = Fishcord.fishcordmods.entity.BobberEntity.State.HOOKED_IN_ENTITY;
                    return;
                }

                if (bl) {
                    this.setVelocity(this.getVelocity().multiply(0.3, 0.2, 0.3));
                    this.state = Fishcord.fishcordmods.entity.BobberEntity.State.BOBBING;
                    return;
                }

                this.checkForCollision();
            } else {
                if (this.state == Fishcord.fishcordmods.entity.BobberEntity.State.HOOKED_IN_ENTITY) {
                    if (this.hookedEntity != null) {
                        if (!this.hookedEntity.isRemoved() && this.hookedEntity.getWorld().getRegistryKey() == this.getWorld().getRegistryKey()) {
                            ...
                            this.state = Fishcord.fishcordmods.entity.BobberEntity.State.FLYING;
                        }
                    }
                ...
                if (this.state == Fishcord.fishcordmods.entity.BobberEntity.State.BOBBING) {
            ...
            this.updateRotation();
            if (this.state == Fishcord.fishcordmods.entity.BobberEntity.State.FLYING && (this.isOnGround() || this.horizontalCollision)) {
                this.setVelocity(Vec3d.ZERO);
            }
    ...
    private boolean isOpenOrWaterAround(BlockPos pos) {
        Fishcord.fishcordmods.entity.BobberEntity.PositionType positionType = Fishcord.fishcordmods.entity.BobberEntity.PositionType.INVALID;

        for (int i = -1; i <= 2; i++) {
            Fishcord.fishcordmods.entity.BobberEntity.PositionType positionType2 = this.getPositionType(pos.add(-2, i, -2), pos.add(2, i, 2));
            switch (positionType2) {
                case ABOVE_WATER:
                    if (positionType == Fishcord.fishcordmods.entity.BobberEntity.PositionType.INVALID) {
                        return false;
                    }
                    break;
                case INSIDE_WATER:
                    if (positionType == Fishcord.fishcordmods.entity.BobberEntity.PositionType.ABOVE_WATER) {
                        return false;
                    }
                    break;
                case INVALID:
    ...
    private Fishcord.fishcordmods.entity.BobberEntity.PositionType getPositionType(BlockPos start, BlockPos end) {
        return (Fishcord.fishcordmods.entity.BobberEntity.PositionType)BlockPos.stream(start, end)
                .map(this::getPositionType)
                .reduce((positionType, positionType2) -> positionType == positionType2 ? positionType : Fishcord.fishcordmods.entity.BobberEntity.PositionType.INVALID)
                .orElse(Fishcord.fishcordmods.entity.BobberEntity.PositionType.INVALID);
    }

    private Fishcord.fishcordmods.entity.BobberEntity.PositionType getPositionType(BlockPos pos) {
        ...
        if (!blockState.isAir() && !blockState.isOf(Blocks.LILY_PAD)) {
            ...
            return fluidState.isIn(FluidTags.WATER) && fluidState.isStill() && blockState.getCollisionShape(this.getWorld(), pos).isEmpty()
                    ? Fishcord.fishcordmods.entity.BobberEntity.PositionType.INSIDE_WATER
                    : Fishcord.fishcordmods.entity.BobberEntity.PositionType.INVALID;
        } else {
            return Fishcord.fishcordmods.entity.BobberEntity.PositionType.ABOVE_WATER;
        }
    }
    ...
    private void setPlayerFishHook(@Nullable Fishcord.fishcordmods.entity.BobberEntity fishingBobber) {
        PlayerEntity playerEntity = this.getPlayerOwner();
        if (playerEntity != null) {
            //playerEntity.fishHook = Bobber;
        }
    }
